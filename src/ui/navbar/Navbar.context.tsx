...
function NavDropdown({ item, size }: { item: NavItem; size: NavbarSize }) {
            ...
            <div className="p-1">
              {item.children!.map((child) => (
                ...
                >
                  {child.icon && (
                    <span
                      className="shrink-0 text-gray-400 dark:text-gray-500 flex items-center"
                      style={{ width: s.iconSize - 2, height: s.iconSize - 2 }}
                    >
                      {child.icon}
                    </span>
                  ...
                  {child.badge !== undefined && (
                    <span
                      className={`shrink-0 ${s.badgeText} font-semibold px-1.5 py-0.5 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400`}
                    >
                      {child.badge}
                    </span>
                  )}
...
function NavTabItem({ item, size }: { item: NavItem; size: NavbarSize }) {
  const s = sizeConfig[size];
  const { state, setActiveTab } = useNavbarContext();
  const isActive = state.activeTab === item.id;
  ...
  return (
    ...
    >
      ...
      {item.badge !== undefined && (
        <span
          className={`shrink-0 ${s.badgeText} font-semibold px-1.5 py-0.5 rounded-full ${isActive ? "bg-white/20 text-white dark:bg-black/20 dark:text-gray-900" : "bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400"}`}
        >
          {item.badge}
        </span>
      )}
...
function SearchBar({ size }: { size: NavbarSize }) {
  ...
  return (
    ...
    >
      <div
        className={`flex items-center gap-2 ${s.searchHeight} ${s.searchPx} rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 min-w-[180px] sm:min-w-[240px]`}
      >
        <Search
          size={s.iconSize - 3}
...
function MobileMenuPanel({
      ...
      return (
        <React.Fragment key={item.id}>
          ...
          >
            {item.icon && (
              <span
                className={`shrink-0 flex items-center ${isActive && !hasChildren ? "opacity-100" : "opacity-50"}`}
                style={{ width: s.iconSize, height: s.iconSize }}
              >
                {item.icon}
              </span>
  ...
  return (
    ...
    >
      <div className="p-2 space-y-0.5">{renderItems(items)}</div>
...
function NavbarInner({
  ...
  const currentVariantStyle = scrolled
    ? variantScrolled[variant]
    : variantStyles[variant];

  return (
    <div
      className={`${positionClass[position]} z-40 w-full transition-all duration-200 ${currentVariantStyle} ${className}`}
    >
      <div className={`w-full mx-auto ${maxWidth}`}>
        <div className={`flex items-center ${s.height} ${s.px} gap-3`}>
          ...
          {items.length > 0 && (
            <nav
              className={`hidden md:flex items-center ${s.gap} flex-1 ${tabsClassName}`}
            >
              {centerContent ? (
          ...
          <div
            className={`flex items-center ${s.gap} ml-auto shrink-0 ${actionsClassName}`}
          >
            <AnimatePresence>
