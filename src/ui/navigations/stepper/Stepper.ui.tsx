...
import { Check, X, AlertTriangle, Minus, ChevronRight } from "lucide-react";
import {
  StepperProvider,
  useStepperContext,
...
const variantConfig = {
  default: {
    wrapper:
      "bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm shadow-black/5 dark:shadow-black/20 p-6",
    // Step indicator states
    indicator: {
      pending:
        "bg-white dark:bg-gray-900 border-2 border-gray-200 dark:border-gray-700 text-gray-400 dark:text-gray-500",
      active:
        "bg-gray-900 dark:bg-white border-2 border-gray-900 dark:border-white text-white dark:text-gray-900",
      completed:
        "bg-gray-900 dark:bg-white border-2 border-gray-900 dark:border-white text-white dark:text-gray-900",
      error:
        "bg-red-50 dark:bg-red-950 border-2 border-red-500 dark:border-red-400 text-red-500 dark:text-red-400",
      warning:
        "bg-amber-50 dark:bg-amber-950 border-2 border-amber-500 dark:border-amber-400 text-amber-600 dark:text-amber-400",
      skipped:
        "bg-white dark:bg-gray-900 border-2 border-dashed border-gray-300 dark:border-gray-600 text-gray-300 dark:text-gray-600",
    },
    // Connector
    ...
    // Content panel
    contentPanel:
      "bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-gray-100 dark:border-gray-800 p-4",
  },
  filled: {
    wrapper:
      "bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm shadow-black/5 dark:shadow-black/20 p-6",
    indicator: {
      pending:
        "bg-gray-100 dark:bg-gray-800 border-2 border-transparent text-gray-400 dark:text-gray-500",
      active:
        "bg-gray-900 dark:bg-white border-2 border-transparent text-white dark:text-gray-900",
      completed:
        "bg-gray-700 dark:bg-gray-200 border-2 border-transparent text-white dark:text-gray-900",
      error:
        "bg-red-500 dark:bg-red-500 border-2 border-transparent text-white",
      warning:
        "bg-amber-500 dark:bg-amber-500 border-2 border-transparent text-white",
      skipped:
        "bg-gray-100 dark:bg-gray-800 border-2 border-transparent text-gray-300 dark:text-gray-600",
    },
    connectorBase: "bg-gray-200 dark:bg-gray-700",
    connectorCompleted: "bg-gray-900 dark:bg-white",
    connectorActive: "bg-gray-900 dark:bg-white",
  ...
  ghost: {
    ...
    indicator: {
      pending:
        "bg-transparent border-2 border-gray-200 dark:border-gray-700 text-gray-400 dark:text-gray-500",
      active:
        "bg-gray-900 dark:bg-white border-2 border-gray-900 dark:border-white text-white dark:text-gray-900",
      completed:
        "bg-transparent border-2 border-gray-400 dark:border-gray-500 text-gray-500 dark:text-gray-400",
      error:
        "bg-transparent border-2 border-red-500 dark:border-red-400 text-red-500 dark:text-red-400",
      warning:
        "bg-transparent border-2 border-amber-500 dark:border-amber-400 text-amber-600 dark:text-amber-400",
      skipped:
        "bg-transparent border-2 border-dashed border-gray-200 dark:border-gray-700 text-gray-300 dark:text-gray-600",
    },
    connectorBase: "bg-gray-200 dark:bg-gray-700",
    ...
    subtitleColor: "text-gray-400 dark:text-gray-500",
    optionalBadge:
      "bg-transparent text-gray-400 dark:text-gray-500 border border-gray-200 dark:border-gray-700",
    contentPanel:
      "bg-transparent border-l-2 border-gray-200 dark:border-gray-700 pl-4",
  },
  outline: {
    wrapper:
      "bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl p-6",
    indicator: {
      pending:
        "bg-white dark:bg-gray-900 border-2 border-gray-200 dark:border-gray-700 text-gray-400 dark:text-gray-500",
      active:
        "bg-white dark:bg-gray-900 border-2 border-gray-900 dark:border-white text-gray-900 dark:text-white",
      completed:
        "bg-white dark:bg-gray-900 border-2 border-gray-400 dark:border-gray-500 text-gray-500 dark:text-gray-400",
      error:
        "bg-white dark:bg-gray-900 border-2 border-red-500 dark:border-red-400 text-red-500 dark:text-red-400",
      warning:
        "bg-white dark:bg-gray-900 border-2 border-amber-500 dark:border-amber-400 text-amber-600 dark:text-amber-400",
      skipped:
        "bg-white dark:bg-gray-900 border-2 border-dashed border-gray-200 dark:border-gray-700 text-gray-300 dark:text-gray-600",
    },
    connectorBase: "bg-gray-200 dark:bg-gray-700",
    ...
    subtitleColor: "text-gray-400 dark:text-gray-500",
    optionalBadge:
      "bg-white dark:bg-gray-900 text-gray-400 dark:text-gray-500 border border-gray-200 dark:border-gray-700",
    contentPanel:
...
function StatusIcon({
  ...
  return (
    <AnimatePresence mode="wait" initial={false}>
      ...
      >
        {status === "completed" &&
          (step.completedIcon ?? <Check size={iconSize} strokeWidth={2.5} />)}
        {status === "error" &&
          (step.errorIcon ?? <X size={iconSize} strokeWidth={2.5} />)}
        {status === "warning" &&
          (step.warningIcon ?? (
...
function Connector({
  ...
  if (orientation === "horizontal") {
    return (
      <div
        className={`flex-1 relative ${s.connectorH} ${v.connectorBase} overflow-hidden ${connectorClassName}`}
      >
        <motion.div
  ...
  return (
    <div
      className={`relative ${s.connectorV} ml-auto mr-auto shrink-0 ${v.connectorBase} overflow-hidden ${connectorClassName}`}
...
function StepLabel({
  ...
  return (
    <div
      className={`flex flex-col ${s.labelGap} ${orientation === "horizontal" ? "items-center text-center" : "items-start text-left"} min-w-0`}
    >
      <div className="flex items-center gap-1.5 flex-wrap">
        <span
          className={`${s.titleText} ${titleColorMap[status]} leading-tight transition-colors duration-200 truncate ${titleClassName}`}
        >
          {step.title}
        </span>
        {step.optional && (
          <span
            className={`${s.optionalText} px-1.5 py-0.5 rounded-full font-medium tracking-wide ${v.optionalBadge} ${optionalClassName}`}
          >
            Optional
          </span>
        )}
      ...
      {step.subtitle && (
        <span
          className={`${s.subtitleText} ${v.subtitleColor} leading-tight truncate ${subtitleClassName}`}
        >
          {step.subtitle}
        </span>
      )}
...
function HorizontalStep({
  ...
  const isCompleted = status === "completed" || status === "skipped";
  const canClick =
    clickable && !step.disabled && !linear
      ? true
      : clickable &&
        !step.disabled &&
        (isCompleted || state.visitedSteps.has(index));
  ...
  return (
    <div className={`flex gap-3 ${stepClassName}`}>
      ...
      <div className="flex flex-col items-center shrink-0">
        ...
        {!isLast && (
          <div
            className={`flex-1 ${s.connectorV} my-1 ${v.connectorBase} overflow-hidden relative ${connectorClassName}`}
          >
            <motion.div
      ...
      <div className={`flex-1 min-w-0 ${isLast ? "pb-0" : "pb-4"}`}>
        ...
        <AnimatePresence initial={false}>
          {isActive && (step.content || step.description) && (
            ...
            >
              <div
                className={`${s.contentGapV} ${v.contentPanel} ${contentPanelClassName}`}
              >
                {step.content ?? (
                  <p
                    className={`${s.descriptionText} text-gray-500 dark:text-gray-400 leading-relaxed`}
                  >
                    {step.description}
                  </p>
                )}
...
function StepperInner({
  ...
  return (
    <div className={`w-full ${v.wrapper} ${wrapperClassName}`}>{inner}</div>
  );
}
